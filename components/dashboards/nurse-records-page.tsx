...
export default async function NurseRecordsPage({ user }: { user: User }) {
  const now = new Date();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Records viewer</h1>
        <p className="text-muted-foreground">Read-only index for assigned patients</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>Records</CardTitle>
          <CardDescription>{records.length} items</CardDescription>
        </CardHeader>
        <CardContent>
          {records.length === 0 ? (
