...
export async function POST(request: NextRequest) {
  try {
    ...
    const apiUrl =
      process.env.BIOMETRIC_SINGLE_VERIFY_URL ||
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          console.log(
            "Single-modality endpoint not found, trying multi-modal...",
          );
          ...
          else if (mappedType === "fingerprint")
            multiModalPayload.fingerprint_image = dataUrl;
          else if (mappedType === "iris")
            multiModalPayload.iris_image = dataUrl;
